...
build {
  ...
  provisioner "shell" {
    inline = [
      ...
      "sudo trivy rootfs --no-progress --scanners vuln --format json --output golden-ubuntu-scan-{{timestamp}}.json /"
    ]
  }

  provisioner "file" {
    source = "golden-ubuntu-scan-{{timestamp}}.json"
    destination = "./golden-ubuntu-scan-{{timestamp}}.json"
    direction = "download"
  }
}